...
def load_events(filepath: str) -> pd.DataFrame:
    ...
    if bad_ts.any():
        log.warning('Dropping %d rows with unparseable timestamps: %s', bad_ts.sum(), df.loc[bad_ts, 'event_id'].tolist())
    df = df[~bad_ts].copy()
    df['timestamp'] = df['timestamp'].dt.tz_localize('America/New_York', ambiguous='infer')
    dupes = df.duplicated(subset='event_id', keep=False)
    if dupes.any():
        log.warning('Dropping %d rows with duplicate event_id values: %s', dupes.sum(), df.loc[dupes, 'event_id'].tolist())
        df = df[~dupes].copy()
    df['event_flag'] = pd.to_numeric(df['event_flag'], errors='coerce')
    bad_flag = df['event_flag'].isna()
    if bad_flag.any():
        log.warning('Dropping %d rows with non-numeric event_flag: %s', bad_flag.sum(), df.loc[bad_flag, 'event_id'].tolist())
        df = df[~bad_flag].copy()
    log.info('Loaded %d valid events.', len(df))
    df = df.sort_values('timestamp').reset_index(drop=True)
    return df

def download_market_data(fetcher: MarketDataFetcher, ticker: str, event_ts: pd.Timestamp, window_minutes: int=90) -> pd.DataFrame:
    log.info('Fetching %s bars around %s (±%d min)', ticker, event_ts, window_minutes)
    df = fetcher.get_bars_around_event(ticker=ticker, event_time=event_ts, window_minutes=window_minutes)
    if df.empty:
...
def extract_event_window_metrics(price_series: pd.Series, event_ts: pd.Timestamp, ticker: str, event_id) -> dict | None:
    ...
    if pre_start_idx < 0:
        log.info("event_id=%s (%s): pre-market event — using previous day's closing bars as pre-event baseline (have %d bars before event in series).", event_id, ticker, event_bar_idx)
    ...
    if post_end_idx > len(price_series):
        log.warning('SKIP event_id=%s (%s): insufficient post-event bars (need %d, have %d after event).', event_id, ticker, BARS_POST_60M, len(price_series) - event_bar_idx)
        return None
    ...
    t = ticker
    return {f'{t}_rv_pre_30m': compute_realized_volatility(pre_returns), f'{t}_rv_post_5m': compute_realized_volatility(post_returns_5m), f'{t}_rv_post_30m': compute_realized_volatility(post_returns_30m), f'{t}_rv_post_60m': compute_realized_volatility(post_returns_60m), f'{t}_ret_5m': compute_cumulative_return(post_5m), f'{t}_ret_30m': compute_cumulative_return(post_30m), f'{t}_ret_60m': compute_cumulative_return(post_60m)}
...
def build_dataset(events_path: str, output_path: str, limit: int=None) -> pd.DataFrame:
    ...
    for ticker in TICKERS:
        ...
        if df.empty:
            log.error('%s: full history not available. Run `python fetch_full_data.py` first.', ticker)
            sys.exit(1)
        full_data[ticker] = df
        log.info('%s: %d bars in memory (%s -> %s)', ticker, len(df), df.index.min().date(), df.index.max().date())
    ...
    for _, row in events.iterrows():
        ...
        row_metrics = {'event_id': event_id, 'timestamp': event_ts, 'event_name': event_name, 'event_category': row['event_category'], 'event_flag': row['event_flag']}
        valid_event = True
        for ticker in TICKERS:
            window_df = slice_event_window(full_df=full_data[ticker], event_ts=event_ts, pre_minutes=90, post_minutes=90)
            if window_df.empty:
                log.warning('SKIP event_id=%s (%s): no bars in +/-90-min window around %s.', event_id, ticker, event_ts)
                valid_event = False
                break
            price_series = window_df['close'].rename('Close')
            metrics = extract_event_window_metrics(price_series=price_series, event_ts=event_ts, ticker=ticker, event_id=event_id)
            if metrics is None:
                valid_event = False
                break
...
def main():
    parser = argparse.ArgumentParser(description='Event-study pipeline: macro announcements × SPY/QQQ volatility.')
    parser.add_argument('--events', default=str(THIS_DIR.parent / 'data' / 'events.csv'), help='Path to the input events CSV (default: ../data/events.csv)')
    parser.add_argument('--output', default='event_market_data.csv', help='Path to write the output CSV (default: event_market_data.csv)')
    parser.add_argument('--limit', type=int, default=None, help='Only process the first N events (default: all). Use for test runs.')
