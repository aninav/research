...
======================
Formal regression analysis for Research Question 2:
"Can categorical and temporal features predict the magnitude of
post-announcement realized volatility in SPY and QQQ?"

This module adds the OLS and panel regression framework that complements the
ML models in model.py.  It is designed to be cited in the paper and produces
publication-ready coefficient tables.

Models estimated
----------------
1. OLS baseline
   RV_post = β0 + β_category * D_category + ε

2. OLS with temporal controls
   RV_post = β0 + β_category * D_category
           + β_year * year + β_quarter * Q + ε

3. OLS with full feature set (replicates model.py feature set but in OLS form)
   RV_post = β0 + β_category + β_entity + β_keyword
           + β_year + β_quarter + β_month + ε

4. Panel regression (pooled OLS with entity fixed effects for ticker)
   RV_post_it = α_i + β * X_it + ε_it
   where i indexes ticker (SPY, QQQ) and t indexes event

5. Interaction model
   RV_post = β0 + β_inflation * D_inflation
           + β_highInfl * D_high_inflation_regime
           + β_interact * (D_inflation × D_high_inflation_regime) + ε
   Tests whether inflation announcements were especially impactful during
   the 2021-2023 high-inflation period.
...
--------------------------------
- table_ols_baseline.csv
- table_ols_temporal.csv
- table_ols_full.csv
- table_panel_ols.csv
- table_interaction.csv
- table_regression_summary.csv   (all models side-by-side)

Usage
-----
    python regression_analysis.py
    python regression_analysis.py --input ../data/master_dataset.csv
...
def build_feature_matrix(df: pd.DataFrame, ticker: str) -> tuple[pd.DataFrame, pd.Series]:
    """
    Construct the full feature matrix X and target y for `ticker`.

    Features
    --------
    Category dummies     : D_inflation, D_labor, D_monetary_policy,
                           D_consumption (growth is the omitted base)
    Entity dummies       : ent_bls, ent_fed, ent_bea, ent_census
    Keyword flags        : kw_inflation, kw_labor, kw_monetary, kw_consumption, kw_growth
    Temporal             : year (continuous), quarter dummies (Q2, Q3, Q4),
                           month (continuous)
    Regime               : is_high_inflation_regime
    Pre-event volatility : {ticker}_rv_pre_30m  (control for baseline vol)
...
def _run_ols(X: pd.DataFrame, y: pd.Series, model_name: str) -> sm.regression.linear_model.RegressionResultsWrapper:
    ...
    log.info(
        ...
        model.fvalue, model.f_pvalue, int(model.nobs),
    )
    return model


def results_to_df(model, model_name: str, ticker: str) -> pd.DataFrame:
    """Convert statsmodels result to a tidy DataFrame row per coefficient."""
    rows = []
    for var in model.params.index:
        rows.append({
            ...
            "sig"        : _sig_stars(model.pvalues[var]),
            "r2"         : model.rsquared,
...
def model_ols_baseline(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    """
    RV_post = β0 + β_inf*D_inf + β_lab*D_lab + β_mon*D_mon + β_con*D_con + ε
    Base category: growth (omitted dummy).
    """
...
def model_ols_temporal(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    """
    RV_post = β0 + β_category*D + β_year*year + β_Q*Q + ε
    """
    sub, y = build_feature_matrix(df, ticker)
    X = sub[[
        "D_inflation", "D_labor", "D_monetary_policy", "D_consumption",
        "year", "Q2", "Q3", "Q4",
    ]]
...
def model_ols_full(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    """
    Full feature set: category dummies + temporal + pre-event vol control
    + keyword / entity dummies (where available).
    """
    sub, y = build_feature_matrix(df, ticker)
    pre_vol_col = f"{ticker}_rv_pre_30m"
    ...
    optional_cols = [
        "kw_inflation", "kw_labor", "kw_monetary", "kw_consumption",
        "ent_bls", "ent_fed", "ent_bea",
    ]
...
def model_panel_ols(df: pd.DataFrame) -> pd.DataFrame:
    """
    Pooled panel: stack SPY and QQQ observations.
    RV_post_it = α_i + β*X_it + ε_it
    Ticker fixed effect via a D_QQQ dummy (SPY is base).
    """
    rows_list = []
    for ticker in TICKERS:
        ...
        sub = df[[
            "event_category", "year", "quarter", "month",
            "is_high_inflation_regime", target_col, pre_vol_col,
        ]].dropna(subset=[target_col, pre_vol_col]).copy()

        sub = sub.rename(columns={
            target_col  : "rv_post_30m",
            pre_vol_col : "rv_pre_30m",
        })
        sub["ticker"]  = ticker
...
def model_interaction(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    """
    Tests whether CPI/inflation releases were disproportionately impactful
    during the 2021-mid-2023 high-inflation period.

    RV_post = β0 + β_inf*D_inf + β_regime*D_regime
            + β_interact*(D_inf × D_regime) + controls + ε

    A positive β_interact would mean that inflation announcements during the
    high-inflation regime caused systematically larger volatility spikes —
    consistent with elevated macro uncertainty amplifying information shocks.
    """
    sub, y = build_feature_matrix(df, ticker)
    pre_vol_col = f"{ticker}_rv_pre_30m"

    sub["D_inflation_x_regime"] = (
        sub["D_inflation"] * sub["is_high_inflation_regime"]
    )

    X = sub[[
        "D_inflation", "D_labor", "D_monetary_policy", "D_consumption",
        "is_high_inflation_regime",
        "D_inflation_x_regime",
        "year", "Q2", "Q3", "Q4",
        pre_vol_col,
    ]]
...
def build_summary_table(all_results: list[pd.DataFrame]) -> pd.DataFrame:
    """
    Build a wide-format summary showing R², adj-R², n, and F-stat
    for each model × ticker combination.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run OLS and panel regression analysis on macro event volatility data. "
            "Requires master_dataset.csv to exist."
        )
    )
    parser.add_argument(
        "--input",
        default=str(MASTER_PATH),
        help=f"Path to master_dataset.csv. Default: {MASTER_PATH}",
    )
    parser.add_argument(
        "--tables-dir",
        default=str(TABLES_DIR),
        help=f"Directory for output tables. Default: {TABLES_DIR}",
    )
    ...
    if not input_path.exists():
        log.error(
            ...
            "Run build_market_dataset.py first to generate master_dataset.csv.",
            input_path,
        )
        sys.exit(1)
