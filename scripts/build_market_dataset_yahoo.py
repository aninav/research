...
def download_market_data(ticker: str, start: str, end: str, interval: str='5m') -> pd.DataFrame:
    log.info('Downloading %s  %s  %s → %s', ticker, interval, start, end)
    raw = yf.download(ticker, start=start, end=end, interval=interval, prepost=True, auto_adjust=True, progress=False)
    if raw.empty:
...
def extract_event_window_metrics(price_series: pd.Series, event_ts: pd.Timestamp, ticker: str, event_id) -> dict | None:
    ...
    if pre_start_idx < 0:
        log.warning('SKIP event_id=%s (%s): insufficient pre-event bars (need %d, have %d).', event_id, ticker, BARS_PRE_30M, event_bar_idx)
        return None
    ...
    for _, row in events.iterrows():
        ...
        for ticker in TICKERS:
            ...
            if price_df.empty:
                ...
                valid_event = False
                break
            metrics = extract_event_window_metrics(price_series=price_df['Close'], event_ts=event_ts, ticker=ticker, event_id=event_id)
            if metrics is None:
                valid_event = False
                break
...
def main():
    ...
    parser.add_argument('--events', default='events.csv', help='Path to the input events CSV (default: events.csv)')
