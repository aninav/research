...
class FullDataFetcher:
    ...
    def fetch_ticker(self, ticker: str, start_date: str=DEFAULT_START_DATE, end_date: str | None=None) -> pd.DataFrame:
        ticker = ticker.upper()
        ...
        if existing is not None:
            ...
            if earliest_cached <= start_dt:
                log.info('%s: cache already covers requested range (earliest bar: %s). Returning cached data.', ticker, earliest_cached)
                return existing
            end_dt = earliest_cached - timedelta(minutes=5)
            log.info('%s: resuming download from %s (earliest cached bar: %s).', ticker, end_dt, earliest_cached)
        ...
        while True:
            ...
            log.info('%s | chunk bars: %d | window: %s → %s | total bars so far: %d', ticker, len(chunk), chunk_earliest.date(), chunk.index.max().date(), sum((len(c) for c in chunks)))
        ...
        log.info('%s: saved %d bars to %s  (%.1f MB)', ticker, len(full_df), out_path, out_path.stat().st_size / 1000000.0)
        return full_df

    def _fetch_chunk(self, ticker: str, end_dt: pd.Timestamp) -> pd.DataFrame | None:
        end_str = end_dt.strftime('%Y-%m-%d %H:%M:%S')
        params = {'symbol': ticker, 'interval': INTERVAL, 'end_date': end_str, 'outputsize': MAX_BARS_PER_CALL, 'timezone': 'America/New_York', 'format': 'JSON', 'apikey': self.api_key}
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            if payload.get('status') == 'error':
                log.error('Twelve Data API error: %s', payload.get('message', 'unknown'))
                return None
            values = payload.get('values')
            if not values:
                return None
            df = self._parse(values)
            log.info('  [%s] chunk fetched: %d bars | %s → %s', ticker, len(df), df.index.min().date(), df.index.max().date())
            return df
        return None
    ...
    def _load_existing(path: Path, ticker: str) -> pd.DataFrame | None:
        ...
        try:
            ...
            log.info('%s: loaded existing cache (%d bars, %s → %s)', ticker, len(df), df.index.min().date(), df.index.max().date())
            return df
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Download full 5-minute history for SPY/QQQ from Twelve Data API and save to data/cache/{TICKER}_full_5min.csv')
    ...
    parser.add_argument('--tickers', nargs='+', default=DEFAULT_TICKERS, help=f'Tickers to download. Default: {DEFAULT_TICKERS}')
    parser.add_argument('--api-key', default=os.environ.get('TWELVE_DATA_API_KEY', ''), help='Twelve Data API key. Defaults to TWELVE_DATA_API_KEY env var.')
    parser.add_argument('--cache-dir', default=str(CACHE_DIR), help=f'Directory for output CSVs. Default: {CACHE_DIR}')
    parser.add_argument('--dry-run', action='store_true', help='Estimate API call count only — do not fetch any data.')
    args = parser.parse_args()
    if args.dry_run:
        estimate_calls(args.start, args.tickers)
        return
    if not args.api_key:
        log.error('No API key provided. Set TWELVE_DATA_API_KEY environment variable or pass --api-key <key>.')
        sys.exit(1)
    fetcher = FullDataFetcher(api_key=args.api_key, cache_dir=Path(args.cache_dir))
    for ticker in args.tickers:
        df = fetcher.fetch_ticker(ticker=ticker, start_date=args.start, end_date=args.end)
        if df.empty:
            log.error('No data downloaded for %s.', ticker)
        else:
            log.info('✓ %s complete: %d bars | %s → %s', ticker, len(df), df.index.min().date(), df.index.max().date())
