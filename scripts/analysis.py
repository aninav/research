...
def rv_cols(ticker: str) -> dict:
    return {'pre_30m': f'{ticker}_rv_pre_30m', 'post_5m': f'{ticker}_rv_post_5m', 'post_30m': f'{ticker}_rv_post_30m', 'post_60m': f'{ticker}_rv_post_60m'}

def ret_cols(ticker: str) -> dict:
    return {'5m': f'{ticker}_ret_5m', '30m': f'{ticker}_ret_30m', '60m': f'{ticker}_ret_60m'}
...
def table2_rv_by_category(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker in TICKERS:
        ...
        for category, grp in df.groupby('event_category'):
            row = {'ticker': ticker, 'event_category': category, 'n_events': len(grp), 'rv_pre_30m': grp[cols['pre_30m']].mean(), 'rv_post_5m': grp[cols['post_5m']].mean(), 'rv_post_30m': grp[cols['post_30m']].mean(), 'rv_post_60m': grp[cols['post_60m']].mean()}
    ...
    for label, subset in subsets.items():
        for ticker in TICKERS:
            ...
            for window in ['post_5m', 'post_30m', 'post_60m']:
                ...
                rows.append({'subset': label, 'ticker': ticker, 'window': window, 'n': len(aligned), 'mean_pre_rv': pre_vals.mean(), 'mean_post_rv': post_vals.mean(), 'mean_diff': (post_vals - pre_vals).mean(), 't_statistic': t_stat, 'p_value_2sided': p_two, 'p_value_1sided': p_one, 'significant_5pct': int(p_one < 0.05), 'significant_1pct': int(p_one < 0.01)})
    result = pd.DataFrame(rows).round(6)
...
def table4_return_by_category(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker in TICKERS:
        ...
        for category, grp in df.groupby('event_category'):
            row = {'ticker': ticker, 'event_category': category, 'n_events': len(grp), 'mean_ret_5m': grp[cols['5m']].mean(), 'mean_ret_30m': grp[cols['30m']].mean(), 'mean_ret_60m': grp[cols['60m']].mean(), 'pct_positive_30m': (grp[cols['30m']] > 0).mean()}
            rows.append(row)
    result = pd.DataFrame(rows).round(6)
...
def figure2_rv_heatmap(df: pd.DataFrame) -> None:
    ...
    for ticker in TICKERS:
        cols = rv_cols(ticker)
        ratios[ticker] = df.groupby('event_category').apply(lambda g: g[cols['post_30m']].mean() / g[cols['pre_30m']].mean())
    ratio_df = pd.DataFrame(ratios).round(2)
    fig, ax = plt.subplots(figsize=(6, 4))
    sns.heatmap(ratio_df, annot=True, fmt='.2f', cmap='RdYlGn_r', center=1.0, ax=ax, linewidths=0.5)
...
def figure3_return_distribution(df: pd.DataFrame) -> None:
    ...
    for ax, ticker in zip(axes, TICKERS):
        ...
        ax.axvline(data.mean(), color='#E8634C', linewidth=1.5, linestyle='-', label=f'Mean: {data.mean():.3f}%')
        ax.set_title(ticker, fontsize=12)
...
def run_analysis() -> None:
    ...
    print(all_events[['ticker', 'window', 'n', 'mean_pre_rv', 'mean_post_rv', 't_statistic', 'p_value_1sided', 'significant_5pct']].to_string(index=False))
