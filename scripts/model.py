...
def get_feature_cols(df: pd.DataFrame) -> list[str]:
    exclude_prefixes = ['SPY_', 'QQQ_', 'event_id', 'timestamp', 'event_name', 'event_category', 'event_flag']
    feature_cols = [c for c in df.columns if not any((c.startswith(p) for p in exclude_prefixes)) and df[c].dtype in [np.float64, np.int64, float, int] and (df[c].notna().mean() > 0.8)]
    return feature_cols
...
def build_models() -> dict:
    return {'Linear Regression (Ridge)': Pipeline([('scaler', StandardScaler()), ('model', Ridge(alpha=1.0))]), 'Random Forest': RandomForestRegressor(n_estimators=200, max_depth=5, min_samples_leaf=3, random_state=42, n_jobs=-1), 'Gradient Boosting': GradientBoostingRegressor(n_estimators=200, max_depth=3, learning_rate=0.05, subsample=0.8, random_state=42)}
...
def evaluate_model(model, X: pd.DataFrame, y: pd.Series, cv: int=5) -> dict:
    if len(X) < cv * 2:
        ...
        return {'rmse': np.sqrt(mean_squared_error(y, y_pred)), 'rmse_std': np.nan, 'mae': mean_absolute_error(y, y_pred), 'r2': r2_score(y, y_pred), 'r2_std': np.nan, 'note': 'train-set (too few rows for CV)'}
    ...
    model.fit(X, y)
    return {'rmse': np.sqrt(-neg_mse.mean()), 'rmse_std': np.sqrt(neg_mse.std()), 'mae': -neg_mae.mean(), 'r2': r2.mean(), 'r2_std': r2.std(), 'note': f'{cv}-fold CV'}
...
def table5_model_performance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker in TICKERS:
        ...
        for model_name, model in models.items():
            ...
            rows.append({'ticker': ticker, 'target': target, 'model': model_name, 'n_samples': len(X), 'n_features': X.shape[1], **metrics})
...
def table6_feature_importance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker in TICKERS:
        ...
        if len(X) < 10:
            continue
        rf = RandomForestRegressor(n_estimators=200, max_depth=5, min_samples_leaf=3, random_state=42, n_jobs=-1)
        rf.fit(X, y)
        importance_df = pd.DataFrame({'ticker': ticker, 'feature': X.columns, 'importance': rf.feature_importances_}).sort_values('importance', ascending=False)
...
def figure5_feature_importance(df: pd.DataFrame) -> None:
    ...
    for ax, ticker in zip(axes, TICKERS):
        ...
        rf = RandomForestRegressor(n_estimators=200, max_depth=5, min_samples_leaf=3, random_state=42, n_jobs=-1)
        rf.fit(X, y)
        importance_df = pd.Series(rf.feature_importances_, index=X.columns).sort_values(ascending=True).tail(10)
        ax.barh(importance_df.index, importance_df.values, color='#4C8BE8', alpha=0.85, edgecolor='white')
