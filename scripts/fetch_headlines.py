...
fetch_headlines.py
==================
Fetches real news headlines for each macroeconomic event using NewsAPI.
Searches for the most relevant headline published on the event date
matching the event type and category.
...
Output
------
    ../data/headlines.csv   (event_id, headline, source, published_at)
...
# ---------------------------------------------------------------------------
# Search query templates per event category
# ---------------------------------------------------------------------------
SEARCH_QUERIES = {
    "inflation"      : "CPI inflation consumer price index",
    "labor"          : "jobs report nonfarm payrolls unemployment",
    "monetary_policy": "Federal Reserve FOMC interest rate decision",
    "growth"         : "GDP gross domestic product economic growth",
    "consumption"    : "retail sales consumer spending",
}

# Fallback query if category not found
...
def fetch_headlines() -> pd.DataFrame:
    ...
    # Load market data to get event list
    ...
    for _, row in events.iterrows():
        ...
        # Date range: event date only
        date_str  = timestamp.date().isoformat()
        from_date = date_str
        to_date   = date_str

        # Pick search query based on category
        ...
        try:
            ...
            if articles:
                ...
                full_text = f"{event_name} {category}"  # fallback to event name
                source = "fallback"
                published = date_str
            ...
            rows.append({
                ...
                "event_name"  : event_name,
                "event_date"  : date_str,
                "headline"    : f"{event_name} {category}",
                "source"      : "error_fallback",
                "published_at": date_str,
            })

        # Respect NewsAPI rate limit (free: 100 requests/day, ~1/sec)
        time.sleep(1.2)
